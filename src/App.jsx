      ...
      <div className="w-full flex flex-col items-center mb-12 gap-8 border-b border-[var(--border)] pb-12 text-center">
        ...
        <div className="flex flex-col md:flex-row gap-3 w-full max-w-2xl font-sans text-[var(--text-main)]">
          <div className="flex-1 flex items-center gap-3 px-5 py-3 border border-[var(--border)] bg-[var(--bg-secondary)]">
            <Search size={16} className="opacity-30 flex-shrink-0 text-[var(--text-main)]" />
          ...
          <div className="flex bg-[var(--bg-secondary)] p-1 border border-[var(--border)] overflow-x-auto no-scrollbar">
            {['Все', 'ПК', 'Мобильные'].map(cat => (
              <button key={cat} onClick={() => setActiveFilter(cat)} className={`flex-1 min-w-[90px] px-4 md:px-6 py-2 text-[9px] font-black uppercase transition-all ${activeFilter === cat ? 'bg-[var(--accent)] text-white shadow-lg' : 'opacity-40 text-[var(--text-main)]'}`}>{cat}</button>
            ))}
          </div>
        </div>
      </div>
          ...
          filteredData.map(item => (
            <div key={item.id} className="group flex flex-col xl:flex-row items-center gap-6 md:gap-8 p-4 border border-[var(--border)] bg-[var(--bg-secondary)] transition-all duration-500 hover:border-[var(--accent)]/40">
              ...
              <div className="w-full xl:w-[220px] xl:border-l border-[var(--border)] xl:pl-8 flex flex-col items-center xl:items-end gap-4">
                <div className="text-center xl:text-right font-sans text-[var(--text-main)]">
                ...
                >
                  {/* ИЗМЕНЕНО: Заменено с "В КОЛЛЕКЦИИ" на "СКАЧАТЬ" */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 px-4 md:px-0 w-full justify-items-center text-center">
        {myPresets.length > 0 ? myPresets.map(item => (
          <div key={item.id} className="space-y-4 group w-full max-w-[280px]">
            ...
            <h3 className="text-xs md:text-sm font-black uppercase text-center tracking-tighter opacity-50 group-hover:opacity-100 transition-opacity text-[var(--text-main)]">{item.name}</h3>
          </div>
        )) : (
          <div className="col-span-full py-20 md:py-40 text-center opacity-10 uppercase font-black tracking-[1em] italic text-[var(--text-main)]">Библиотека пуста</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-5xl mx-auto py-16 md:py-24 px-6 flex flex-col items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-center w-full">
        {[
          {s:"01", t:"Выбор стиля", d:"Перейдите в каталог и выберите подходящий набор пресетов. Используйте слайдер 'До/После' для предпросмотра."},
          ...
          {s:"03", t:"Установка и экспорт", d:"Скачайте файлы (.XMP для ПК или .DNG для Mobile) и импортируйте их в Adobe Lightroom."}
        ].map((step, i) => (
          <div key={i} className="space-y-6 p-10 border border-[var(--border)] bg-[var(--bg-secondary)] flex flex-col items-center">
            ...
            <h3 className="text-xl font-black uppercase tracking-tighter text-[var(--text-main)]">{step.t}</h3>
            <p className="opacity-60 text-[11px] leading-relaxed uppercase tracking-wider font-medium text-[var(--text-main)]">{step.d}</p>
          </div>
        ))}
      </div>
    </div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 font-sans bg-[var(--bg-primary)] text-[var(--text-main)]">
      ...
      {/* ГЛАВНЫЙ КОНТЕНТ */}
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10 text-[var(--text-main)]">
        <Routes>
          <Route path="/" element={
            <>
              <div className="py-12 md:py-32 flex flex-col xl:flex-row items-center justify-between gap-12 xl:-mt-10 max-w-6xl mx-auto">
                <div className="w-full xl:w-[48%] space-y-8 md:space-y-12 text-center xl:text-left flex flex-col items-center xl:items-start">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 justify-center xl:justify-start font-sans">
                    ...
                    <Link to="/guide" className={`${btnClass} bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-main)] hover:bg-[var(--text-main)] hover:text-[var(--bg-primary)] transition-all text-center`}>Узнать больше</Link>
                  </div>
                </div>
              ...
              <div className="py-20 md:py-40 border-t border-[var(--border)] max-w-6xl mx-auto">
                <div className="mb-16 text-center">
                  ...
                  <p className="max-w-2xl mx-auto text-[10px] md:text-xs font-black uppercase tracking-[0.4em] text-[var(--accent)] opacity-80 leading-relaxed text-center">
                    Вопросы и ответы
                  </p>
                </div>
      ...
      <footer className="mt-20 border-t border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text-main)]">
        <div className="max-w-7xl mx-auto px-8 py-16 grid grid-cols-1 md:grid-cols-4 gap-12 text-center md:text-left justify-items-center md:justify-items-start">
          <div className="space-y-6">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-60 leading-relaxed italic">
              Автоматизация цвета <br/> для контент-мейкеров.
            </p>
          </div>
          ...
          <div className="flex flex-col items-center md:items-start space-y-6">
            <h4 className="text-[var(--accent)] font-[1000] uppercase text-[11px] tracking-widest">Сети</h4>
        ...
        <div className="border-t border-[var(--border)] py-6 text-center flex flex-col items-center gap-4">
      ...
      {payModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[150] flex items-center justify-center p-4">
          <div className="bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-main)] w-full max-w-lg p-8 md:p-16 shadow-2xl relative animate-in zoom-in-95 duration-300 font-sans flex flex-col items-center text-center">
