...
const GlobalLoader = () => (
  <div className="fixed inset-0 z-[300] bg-[var(--bg-primary)] flex items-center justify-center font-sans text-[var(--text-main)]">
...
const FAQStaticItem = ({ question, answer }) => {
  return (
    <div className="border border-[var(--border)] bg-[var(--bg-secondary)] rounded-sm overflow-hidden h-fit text-[var(--text-main)]">
      <div className="p-6">
...
function Shop({ data, loading, setPayModal, setCurrentProduct, purchasedIds }) {
  ...
  return (
    <div className="py-8 md:py-12 animate-in fade-in duration-700 max-w-6xl mx-auto flex flex-col items-center px-4 overflow-x-hidden">
        ...
        <div className="flex flex-col md:flex-row gap-3 w-full max-w-2xl font-sans">
          ...
          <div className="flex bg-[var(--bg-secondary)] p-1 border border-[var(--border)] overflow-x-auto no-scrollbar rounded-sm">
            {['Все', 'ПК', 'Мобильные'].map(cat => (
              <button key={cat} onClick={() => setActiveFilter(cat)} className={`flex-1 min-w-[80px] md:min-w-[90px] px-3 md:px-6 py-2 text-[9px] font-black uppercase transition-all rounded-sm ${activeFilter === cat ? 'bg-[var(--accent)] text-white shadow-lg' : 'opacity-40'}`}>{cat}</button>
            ))}
          </div>
        </div>
      </div>

      <div className="grid grid-cols-1 gap-6 w-full text-[var(--text-main)]">
        {loading ? [...Array(3)].map((_, i) => <SkeletonCard key={i} />) : 
          filteredData.map(item => (
            <div key={item.id} className="group flex flex-col xl:flex-row items-center gap-6 md:gap-8 p-4 border border-[var(--border)] bg-[var(--bg-secondary)] hover:border-[var(--accent)]/40 transition-all duration-500 rounded-sm">
...
function Profile({ purchasedIds, data }) {
  ...
  return (
    <div className="min-h-screen py-12 md:py-24 max-w-6xl mx-auto flex flex-col items-center px-4 text-[var(--text-main)]">
      <h1 className="text-4xl md:text-7xl font-black uppercase tracking-tighter italic mb-10 border-b border-[var(--border)] pb-10 text-center w-full">Библиотека.</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 px-4 md:px-0 w-full justify-items-center text-center">
        {myPresets.length > 0 ? myPresets.map(item => (
          <div key={item.id} className="space-y-4 group w-full max-w-[280px]">
            ...
            <h3 className="text-xs md:text-sm font-black uppercase tracking-tighter opacity-50">{item.name}</h3>
          </div>
        )) : (
          <div className="col-span-full py-20 opacity-10 uppercase font-black tracking-[1em] italic text-center">Архив пуст</div>
        )}
      </div>
    </div>
  );
...
function Guide() {
  return (
    <div className="max-w-5xl mx-auto py-16 md:py-24 px-6 flex flex-col items-center text-[var(--text-main)] px-4">
...
export default function App() {
  ...
  async function fetchPresets() {
    try {
      ...
      // Изменено: Увеличена загрузка до 3 секунд
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 font-sans bg-[var(--bg-primary)] text-[var(--text-main)] overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-[100] border-b border-[var(--border)] bg-[var(--bg-primary)]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6 flex justify-between items-center relative overflow-x-hidden text-[var(--text-main)]">
          ...
          <div className="flex gap-4 md:gap-8 items-center flex-shrink-0 z-10">
            <button onClick={() => setDarkMode(!darkMode)} className="p-2 transition-all text-[var(--accent)]">
              {darkMode ? <Sun size={20} /> : <Moon size={20} />}
            </button>
            {/* Исправлена кнопка меню */}
            <button onClick={() => setMenuOpen(true)} className="xl:hidden p-1 text-[var(--accent)] focus:outline-none">
              <Menu size={28} />
            </button>
        ...
        {menuOpen && (
          <div className="fixed inset-0 z-[200] bg-[var(--bg-primary)] p-6 flex flex-col items-center justify-center animate-in slide-in-from-right duration-300">
            <button onClick={() => setMenuOpen(false)} className="absolute top-6 right-6 text-[var(--accent)]">
              <X size={32} />
            </button>
            <div className="flex flex-col gap-8 text-center text-2xl font-black uppercase tracking-widest text-[var(--text-main)] overflow-x-hidden">
              <Link to="/catalog" onClick={() => setMenuOpen(false)}>Магазин</Link>
              <Link to="/profile" onClick={() => setMenuOpen(false)}>Библиотека</Link>
              <Link to="/guide" onClick={() => setMenuOpen(false)}>Инструкция</Link>
              <button onClick={() => { setDarkMode(!darkMode); setMenuOpen(false); }} className="text-[var(--accent)]">
            ...
            <>
              ...
              <section className="relative h-[60vh] md:h-[80vh] w-full overflow-hidden flex flex-col items-center justify-center text-center text-[var(--text-main)]">
                ...
                <div className="relative z-10 space-y-8 px-4">
                  ...
                  {/* ИСПРАВЛЕНА СИММЕТРИЯ КНОПОК */}
                  <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                    <Link to="/catalog" className={`${btnClass} bg-[var(--accent)] text-white shadow-2xl shadow-red-500/20 w-full max-w-[280px]`}>
                      Открыть магазин
                    </Link>
                    <Link to="/guide" className={`${btnClass} bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-main)] hover:bg-[var(--text-main)] hover:text-[var(--bg-primary)] w-full max-w-[280px]`}>
                      Как это работает?
                    </Link>
                  </div>
                </div>
              ...
              <div className="py-12 md:py-40 border-t border-[var(--border)] max-w-6xl mx-auto px-4 text-[var(--text-main)]">
                <div className="mb-16 text-center">
                  <h3 className="text-4xl md:text-6xl font-[1000] uppercase tracking-tighter italic leading-none mb-6">FAQ.</h3>
