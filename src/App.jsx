        ...
        <div className="flex flex-col md:flex-row gap-3 w-full max-w-2xl font-sans text-[var(--text-main)]">
          ...
          <div className="flex bg-[var(--bg-secondary)] p-1 border border-[var(--border)] overflow-x-auto no-scrollbar">
            {['Все', 'ПК', 'Мобильные'].map(cat => (
              <button key={cat} onClick={() => setActiveFilter(cat)} className={`flex-1 min-w-[90px] px-4 md:px-6 py-2 text-[9px] font-black uppercase transition-all ${activeFilter === cat ? 'bg-[var(--accent)] text-white' : 'opacity-40'}`}>{cat}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 w-full">
        {loading ? [...Array(3)].map((_, i) => <SkeletonCard key={i} />) : 
          filteredData.map(item => (
            <div key={item.id} className="group flex flex-col xl:flex-row items-center gap-6 md:gap-8 p-4 border border-[var(--border)] bg-[var(--bg-secondary)] hover:border-[var(--accent)]/40 transition-all duration-500">
    ...
    <div className="max-w-5xl mx-auto py-16 md:py-24 px-6 flex flex-col items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-center w-full">
        {[
          ...
          {s:"02", t:"Мгновенный доступ", d:"После оплаты выбранные пресеты мгновенно разблокируются в вашей библиотеке."},
          {s:"03", t:"Установка", d:"Скачайте файлы (.XMP или .DNG) и импортируйте их в Adobe Lightroom."}
        ].map((step, i) => (
          <div key={i} className="space-y-6 p-10 border border-[var(--border)] bg-[var(--bg-secondary)] flex flex-col items-center">
            ...
            <p className="opacity-60 text-[11px] leading-relaxed uppercase font-medium">{step.d}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 font-sans bg-[var(--bg-primary)] text-[var(--text-main)]">
      ...
      <nav className="sticky top-0 z-[100] border-b border-[var(--border)] bg-[var(--bg-primary)]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-5 md:py-8 flex justify-between items-center relative">
          ...
          <Route path="/" element={
            <>
              <div className="py-12 md:py-32 flex flex-col xl:flex-row items-center justify-between gap-12 xl:-mt-10 max-w-6xl mx-auto">
                <div className="w-full xl:w-[48%] space-y-8 text-center xl:text-left flex flex-col items-center xl:items-start">
                  ...
                  <p className="max-w-md opacity-60 text-sm md:text-base font-medium leading-relaxed uppercase tracking-wider">Создавайте профессиональный контент в один клик. Все пресеты разработаны для Adobe Lightroom.</p>
                  <div className="flex flex-col sm:flex-row gap-4 w-full justify-center xl:justify-start">
                    <Link to="/catalog" className={`${btnClass} bg-[var(--accent)] text-white shadow-2xl shadow-red-500/20`}>Каталог стилей</Link>
                    <Link to="/guide" className={`${btnClass} bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-main)] hover:bg-[var(--text-main)] hover:text-[var(--bg-primary)]`}>Узнать больше</Link>
                  </div>
                </div>
                <div className="w-full xl:w-[50%]">
                  <div className="aspect-square bg-[var(--bg-secondary)] border border-[var(--border)] shadow-2xl relative overflow-hidden">
                    <ReactCompareSlider itemOne={<ReactCompareSliderImage src={heroAsset?.before_url || "https://images.unsplash.com/photo-1506744038136-46273834b3fb?q=80&w=2070"} />} itemTwo={<ReactCompareSliderImage src={heroAsset?.after_url || "https://images.unsplash.com/photo-1506744038136-46273834b3fb?q=80&w=2070&sat=-100"} />} className="h-full w-full object-cover transition-all duration-1000" />
                  </div>
                </div>
              </div>
              ...
              <div className="py-20 md:py-40 border-t border-[var(--border)] max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                  <div className="space-y-6">
                    ...
                    <FAQStaticItem question="Нужна ли подписка на Lightroom?" answer="Нет, пресеты работают в бесплатной мобильной версии." />
                  </div>
                  <div className="space-y-6">
        ...
        <div className="max-w-7xl mx-auto px-8 py-20 grid grid-cols-1 md:grid-cols-4 gap-12 text-center md:text-left justify-items-center md:justify-items-start">
          <div className="space-y-6">
            <div className="flex items-center gap-3 text-[var(--accent)]">
              <div className="bg-[var(--accent)] text-white w-10 h-10 flex items-center justify-center font-black text-xl">L</div>
              <span className="text-2xl tracking-tighter font-black uppercase">Presets.</span>
            </div>
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-40">Автоматизация цвета.</p>
          </div>
      ...
      {payModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[150] flex items-center justify-center p-4">
          <div className="bg-[var(--bg-secondary)] border border-[var(--border)] w-full max-w-lg p-8 md:p-16 shadow-2xl relative animate-in zoom-in-95 duration-300 flex flex-col items-center text-center">
            ...
            <div className="space-y-10 w-full">
              ...
              <div className={`space-y-10 transition-opacity duration-500 w-full ${isPaying ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
                <input type="text" maxLength="19" placeholder="НОМЕР КАРТЫ" className="w-full bg-transparent border-b border-[var(--border)] py-4 outline-none font-mono text-center focus:border-[var(--accent)]" onChange={(e) => {e.target.value = e.target.value.replace(/\W/gi, '').replace(/(.{4})/g, '$1 ').trim();}} />
                <div className="grid grid-cols-2 gap-8">
                  <input type="text" maxLength="5" placeholder="ММ / ГГ" className="bg-transparent border-b border-[var(--border)] py-4 outline-none text-center focus:border-[var(--accent)]" />
                  <input type="password" maxLength="3" placeholder="CVC" className="bg-transparent border-b border-[var(--border)] py-4 outline-none text-center focus:border-[var(--accent)]" />
                </div>
              </div>
