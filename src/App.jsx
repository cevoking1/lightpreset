...
return (
    <div className="py-8 md:py-12 animate-in fade-in duration-700 max-w-6xl mx-auto flex flex-col items-center px-4">
    <div className="w-full flex flex-col items-center mb-12 gap-8 border-b border-[var(--border)] pb-12 text-center text-[var(--text-main)]">
        ...
        <div className="flex-1 flex items-center gap-3 px-5 py-3 border border-[var(--border)] bg-[var(--bg-secondary)] rounded-sm">
        ...
        <div className="flex bg-[var(--bg-secondary)] p-1 border border-[var(--border)] overflow-x-auto no-scrollbar rounded-sm">
            {['Все', 'ПК', 'Мобильные'].map(cat => (
            <button key={cat} onClick={() => setActiveFilter(cat)} className={`flex-1 min-w-[80px] md:min-w-[90px] px-4 md:px-6 py-2 text-[9px] font-black uppercase transition-all rounded-sm ${activeFilter === cat ? 'bg-[var(--accent)] text-white shadow-lg' : 'opacity-40'}`}>{cat}</button>
            ))}
        </div>
        </div>
    </div>
    ...
    <div className="grid grid-cols-1 gap-6 w-full">
        ...
        filteredData.map(item => (
            <div key={item.id} className="group flex flex-col xl:flex-row items-center gap-6 md:gap-8 p-4 border border-[var(--border)] bg-[var(--bg-secondary)] hover:border-[var(--accent)]/40 transition-all duration-500 text-[var(--text-main)] rounded-sm">
            <div className="w-full xl:w-[280px] aspect-video xl:aspect-[4/3] overflow-hidden bg-black flex-shrink-0 relative border border-[var(--border)] rounded-sm">
            ...
            <div className="w-full xl:w-[220px] xl:border-l border-[var(--border)] xl:pl-8 flex flex-col items-center xl:items-end gap-4">
                ...
                className={`${btnClass} w-full rounded-sm ${purchasedIds.includes(item.id) ? 'bg-[var(--bg-primary)] border border-[var(--border)] text-[var(--text-main)] opacity-50' : 'bg-[var(--accent)] text-white'}`}
                >
...
return (
    <div className="min-h-screen py-12 md:py-24 max-w-6xl mx-auto flex flex-col items-center px-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 px-4 md:px-0 w-full justify-items-center text-center">
        ...
        <div key={item.id} className="space-y-4 group w-full max-w-[280px]">
            <div className="relative aspect-[4/5] overflow-hidden border border-[var(--border)] bg-[var(--bg-secondary)] rounded-sm">
...
function Guide() {
return (
    <div className="max-w-5xl mx-auto py-16 md:py-24 px-6 flex flex-col items-center text-[var(--text-main)]">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-center w-full">
        ...
        <div key={i} className="space-y-6 p-10 border border-[var(--border)] bg-[var(--bg-secondary)] flex flex-col items-center rounded-sm">
...
return (
    <div className="min-h-screen transition-colors duration-300 font-sans bg-[var(--bg-primary)] text-[var(--text-main)] overflow-x-hidden">
    ...
    <nav className="sticky top-0 z-[100] border-b border-[var(--border)] bg-[var(--bg-primary)]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6 flex justify-between items-center relative overflow-x-hidden">
        <Link to="/" className="flex items-center gap-2 group flex-shrink-0 z-10 transition-all hover:opacity-80 rounded-sm">
            <div className="bg-[var(--accent)] text-white w-9 h-9 md:w-11 md:h-11 flex items-center justify-center font-black text-xl shadow-lg rounded-sm">L</div>
        ...
        <div className="flex gap-4 md:gap-8 items-center flex-shrink-0 z-10 text-[var(--text-main)]">
            <button onClick={() => setDarkMode(!darkMode)} className="p-2 transition-all text-[var(--accent)] rounded-sm">
            {darkMode ? <Sun size={20} /> : <Moon size={20} />}
            </button>
            <button onClick={() => setMenuOpen(true)} className="xl:hidden p-1 text-[var(--accent)] focus:outline-none rounded-sm">
            <Menu size={28} />
            </button>
            <Link to="/profile" className="hidden md:flex items-center gap-4 bg-[var(--accent)] text-white px-6 py-2.5 rounded-sm text-[10px] font-black uppercase tracking-widest active:scale-95 shadow-xl shadow-red-500/10">
        ...
        <div className={`fixed inset-x-0 top-0 z-[200] bg-[var(--bg-primary)] border-b border-[var(--border)] shadow-2xl transition-all duration-0 transform ${menuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
  <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6 flex flex-col">
    <div className="flex justify-between items-center w-full">
      <span className="font-black text-xl tracking-tighter uppercase text-[var(--accent)]">Меню.</span>
      <button onClick={() => setMenuOpen(false)} className="text-[var(--accent)] p-1 focus:outline-none">
        <X size={28} />
      </button>
    </div>
    <div className="flex flex-col gap-6 mt-12 text-lg font-black uppercase tracking-widest text-[var(--text-main)]">
      <Link to="/catalog" onClick={() => setMenuOpen(false)} className="py-2 border-b border-[var(--border)]">Магазин</Link>
      <Link to="/profile" onClick={() => setMenuOpen(false)} className="py-2 border-b border-[var(--border)]">Библиотека</Link>
      <Link to="/guide" onClick={() => setMenuOpen(false)} className="py-2 border-b border-[var(--border)]">Инструкция</Link>
    </div>
  </div>
</div>
    </nav>
    ...
    <div className="relative z-10">
        ...
        <Route path="/" element={
            <>
            {/* СЕКЦИЯ 1: VIDEO HERO */}
            <section className="relative h-[60vh] md:h-[80vh] w-full overflow-hidden flex flex-col items-center justify-center text-center text-[var(--text-main)]">
                <video 
                ...
                <h1 className="text-6xl md:text-8xl xl:text-[10rem] font-[1000] uppercase leading-[0.8] tracking-tight">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                    <Link to="/catalog" className={`${btnClass} bg-[var(--accent)] text-white shadow-2xl shadow-red-500/20 w-full max-w-[280px] rounded-sm`}>
                    Открыть магазин
                    </Link>
                    <Link to="/guide" className={`${btnClass} bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-main)] hover:bg-[var(--text-main)] hover:text-[var(--bg-primary)] w-full max-w-[280px] rounded-sm`}>
                    Как это работает?
                    </Link>
                </div>
                </div>
            ...
            {/* СЕКЦИЯ 2: СЛАЙДЕР И ТЕКСТ */}
            <section className="py-12 md:py-24 border-t border-[var(--border)] max-w-7xl mx-auto px-4 text-[var(--text-main)]">
                ...
                <div className="w-full xl:w-[45%] space-y-6 text-center xl:text-left">
                    <h2 className="text-4xl md:text-6xl font-[1000] uppercase tracking-tighter italic leading-none">
                        Визуальное <br/> <span className="text-[var(--accent)]">совершенство.</span>
                    </h2>
                    <p className="opacity-60 text-sm md:text-base leading-relaxed uppercase tracking-widest font-medium">
                ...
                <div className="w-full xl:w-[50%] aspect-square bg-[var(--bg-secondary)] border border-[var(--border)] shadow-2xl relative overflow-hidden rounded-sm">
                    <ReactCompareSlider 
                    ...
                    className="h-full w-full object-cover transition-all duration-1000 rounded-sm" 
                    />
                </div>
                </div>
            </section>

            <div className="py-12 md:py-40 border-t border-[var(--border)] grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10 max-w-6xl mx-auto px-4 text-[var(--text-main)]">
                {[
                ...
                <div key={i} className="p-8 border border-[var(--border)] bg-[var(--bg-secondary)] space-y-6 hover:border-[var(--accent)]/30 transition-colors text-center md:text-left rounded-sm">
                    <MessageSquare className="text-[var(--accent)] mx-auto md:mx-0 rounded-sm" size={20} />
            ...
            {/* FAQ */}
            <div className="py-12 md:py-40 border-t border-[var(--border)] max-w-6xl mx-auto px-4 text-[var(--text-main)]">
                <div className="mb-16 text-center">
                <h3 className="text-3xl md:text-6xl font-[1000] uppercase tracking-tighter italic leading-none mb-6">FAQ.</h3>
                <p className="max-w-2xl mx-auto text-[10px] md:text-xs font-black uppercase tracking-[0.4em] text-[var(--accent)] opacity-80 leading-relaxed text-center">Вопросы и ответы</p>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <FAQStaticItem question="Безопасность платежных данных" answer="Все платежи проходят через зашифрованный шлюз банка-эквайера." />
    ...
    <footer className="mt-20 border-t border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text-main)]">
        <div className="max-w-7xl mx-auto px-8 py-12 md:py-20 grid grid-cols-1 md:grid-cols-4 gap-12 text-center md:text-left justify-items-center md:justify-items-start">
        <div className="space-y-6">
            <div className="flex items-center gap-3 text-[var(--accent)]">
            <div className="bg-[var(--accent)] text-white w-10 h-10 flex items-center justify-center font-black text-xl shadow-lg rounded-sm">L</div>
            <span className="text-2xl tracking-tighter font-black uppercase">Presets.</span>
            </div>
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-40 leading-relaxed italic text-center md:text-left">Автоматизация цвета.</p>
        </div>
        <div className="space-y-4 md:space-y-6 flex flex-col items-center md:items-start text-[10px] font-black uppercase tracking-widest opacity-50">
        ...
        <div className="border-t border-[var(--border)] py-10 flex flex-col items-center gap-4 text-center">
            <Link to="/admin" className="text-[9px] font-black uppercase tracking-[0.5em] opacity-10 hover:opacity-100 hover:text-[var(--accent)] transition-all duration-700 rounded-sm">Admin Terminal</Link>
            <p className="text-[8px] font-black uppercase tracking-[0.4em] opacity-20 text-center">© LightPresets System. All rights reserved.</p>
        </div>
    </footer>
    ...
    {payModal && (
        ...
        <div className="bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text-main)] w-full max-w-lg p-8 md:p-16 shadow-2xl relative animate-in zoom-in-95 duration-300 flex flex-col items-center text-center rounded-sm">
            ...
            <div className={`space-y-8 transition-opacity duration-500 ${isPaying ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
                <input type="text" maxLength="19" placeholder="НОМЕР КАРТЫ" className="w-full bg-transparent border-b border-[var(--border)] py-4 outline-none font-mono text-base md:text-lg focus:border-[var(--accent)] text-[var(--text-main)] text-center rounded-sm" onChange={(e) => {e.target.value = e.target.value.replace(/\W/gi, '').replace(/(.{4})/g, '$1 ').trim();}} />
                <div className="grid grid-cols-2 gap-8">
                <input type="text" maxLength="5" placeholder="ММ / ГГ" className="bg-transparent border-b border-[var(--border)] py-4 outline-none text-sm focus:border-[var(--accent)] text-[var(--text-main)] text-center rounded-sm" />
                <input type="password" maxLength="3" placeholder="CVC" className="bg-transparent border-b border-[var(--border)] py-4 outline-none text-sm focus:border-[var(--accent)] text-center rounded-sm" />
                </div>
            </div>
            <button disabled={isPaying} onClick={() => { setIsPaying(true); setTimeout(() => { setPurchasedIds([...purchasedIds, currentProduct.id]); setPayModal(false); setIsPaying(false); showToast("Пакет успешно добавлен в архив"); }, 2500); }} className={`${btnClass} w-full bg-[var(--accent)] text-white rounded-sm`}>
